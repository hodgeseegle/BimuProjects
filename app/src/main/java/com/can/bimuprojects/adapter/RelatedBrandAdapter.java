...
public class RelatedBrandAdapter extends BaseAdapter {
    ...
    private List<RelatedBrandResponse.BrandsBean> list;

    public RelatedBrandAdapter(Context context, List<RelatedBrandResponse.BrandsBean> list){
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final RelatedBrandResponse.BrandsBean bean = list.get(position);
        if(Util.isOnMainThread())
        Glide.with(context).load(bean.getBrand_background()).placeholder(R.drawable.loading).centerCrop().into(viewHolder.iv);
        if(Util.isOnMainThread())
